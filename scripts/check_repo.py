...
def check_marketplace_manifest(root: Path) -> list[str]:
    """The Claude marketplace must match what the package manifests describe.

    The file is generated by ``scripts/sync_marketplace.py``. A stale one is not
    a cosmetic drift: it is either a package this repository ships and nobody
    can install, or an entry whose install ends in "No manifest found in
    directory". Both are discovered by a person trying to install something,
    which is the worst place to discover either.

    The import is local, matching the bundler checks below: that module imports
    nothing from here, but keeping every generated-artifact check on the same
    pattern is what stops the next one from being a module-scope cycle.
    """
    scripts_dir = str(Path(__file__).resolve().parent)
    if scripts_dir not in sys.path:
        sys.path.insert(0, scripts_dir)
    from sync_marketplace import check_marketplace  # noqa: PLC0415  (local by design)
