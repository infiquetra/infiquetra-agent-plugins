...
The skill does not vendor a virtual environment. Runtime dependency is PyYAML
(`pyyaml>=6.0`); development extras are pytest, ruff, mypy, and types-pyyaml.
See [README.md](README.md) and `scripts/pyproject.toml`.

```bash
python3 scripts/team-scaffold --help
```

`HOME_LAB_ROOT` is the private infrastructure checkout (default `~/home-lab`).
`INFIQUETRA_CONTEXT_LIBRARY` is the context-library checkout (default
`~/infiquetra-context-library`). `register-host` reads
`$HOME_LAB_ROOT/ansible/inventory/hosts.yml`. `stamp` reads the context library.
Neither default is a path on a particular machine.
