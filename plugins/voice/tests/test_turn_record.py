...
class ConcurrencyAndInterleavingTests(TurnRecordTestCase):
    ...
    def test_deterministic_interleaving_prevents_lost_updates(self) -> None:
        """Two concurrent mutate calls from separate threads with event pauses.

        Writer 1 enters critical section and waits on an event. Writer 2 begins
        mutate() in another thread. Writer 2 cannot acquire the flock until Writer 1
        finishes. When Writer 1 finishes and releases, Writer 2 enters and reads
        Writer 1's write, resulting in both updates persisting.
        """
        turn_record.init_turn(SESSION_ONE, BINDING_ID, TURN_ID)

        writer1_in_critical = threading.Event()
        writer2_started = threading.Event()
        writer1_can_finish = threading.Event()

        def writer1_fn(current: turn_record.TurnRecord | None) -> turn_record.TurnRecord:
            writer1_in_critical.set()
            writer2_started.wait(timeout=5.0)
            writer1_can_finish.wait(timeout=5.0)
        ...
        t1.start()
        # Wait until writer 1 is holding the critical section
        self.assertTrue(writer1_in_critical.wait(timeout=5.0))

        # Launch writer 2 (it will block on the flock)
        t2.start()
        time.sleep(0.05)  # allow writer 2 thread to run and enter flock retry loop

        # Now release writer 1
        writer1_can_finish.set()
    ...
    def test_lock_acquisition_timeout_raises_turn_record_busy(self) -> None:
        ...
        try:
            # Use small timeout for test speed (e.g. 0.05s)
            ...
            self.assertFalse(mutation_ran, "mutation function must not run if lock is not acquired")
        finally:
