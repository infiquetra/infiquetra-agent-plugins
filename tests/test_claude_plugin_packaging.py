...
class MarketplaceTests(unittest.TestCase):
    ...
    def test_the_committed_file_is_the_generated_one(self) -> None:
        """The listing rule lives in the generator, and is checked, not restated.

        `scripts/sync_marketplace.py` decides what the marketplace says: a
        package is listed exactly when it carries `.claude-plugin/plugin.json`,
        and every field of an entry is read from that package's own manifests.
        Re-deriving that rule here would be a second copy of it, and two copies
        of one rule can disagree -- which for this file means either a package
        nobody can install or an entry whose install ends in "No manifest found
        in directory".

        So this asserts the committed bytes are the generated bytes. A package
        added without running the generator fails here and at the repository
        gate, and the fix is to run the generator rather than to edit JSON.
        """
        self.assertEqual(
            MARKETPLACE.read_text(encoding="utf-8"),
            sync_marketplace.render(sync_marketplace.build_marketplace(ROOT)),
            "the committed marketplace is not what the package manifests "
            "describe; run `python3 scripts/sync_marketplace.py`",
        )

    def test_the_generator_and_the_derived_subject_agree(self) -> None:
        """The two ways of naming the installable set must answer the same.

        This module derives the subject from the tree to decide what to check;
        the generator derives it again to decide what to list. They are separate
        readings of the same fact, and a difference between them would mean this
        module is checking packages the catalog does not ship, or the reverse.
        """
        self.assertEqual(
            [package.name for package in sync_marketplace.claude_packages(ROOT)],
            [package.name for package in claude_packages()],
        )

    def test_the_repository_gate_fails_on_a_stale_marketplace(self) -> None:
        """The generated file is only safe if something notices when it is stale."""
        self.assertEqual(sync_marketplace.check_marketplace(ROOT), [])
