...
**KTD7 — The portable catalog's minimum supported Python is `python>=3.12`.** It is a minimum and not a pin: every later interpreter is in contract, and raising the floor further is a separate operator decision. The floor is `python>=3.12` because the authoritative source repository, `infiquetra-claude-plugins`, declares and tests exactly that — `requires-python = ">=3.12"` in its project file and `python-version: "3.12"` across its continuous-integration jobs, verified at commit `ed72f439`, the same commit both packages here are derived from — and because a derived catalog must not promise more compatibility than the source it is derived from. The floor is stated once, in `tests/test_python_floor.py`, and every place the catalog declares it is checked against that one value, so the declarations cannot drift apart by hand. Rejected: pinning exactly 3.12, which would refuse interpreters the source supports; and keeping a lower floor, which the ten-client assessment never proved and which the cycle's own Fleet Core re-synchronization broke within one release.

**KTD7's superseded rationale, preserved.** The decision point originally read, in full:
...
That reasoning is superseded rather than wrong on its own terms: reading the *lowest interpreter the ported code happens to parse under* does establish a lower bound, but it is not the same question as *what the catalog promises to support*. The two came apart the moment Fleet Core 0.25.1 landed `from datetime import UTC`, which needs 3.11 and above, and the derived catalog inherited an interpreter requirement its own documentation contradicted while every digest check still passed. A floor read off today's bytes is re-derived by every byte copy; a floor inherited from the authoritative source is stable across them.

**KTD7's frontmatter claim was never true, and cannot be made true downstream.** Both portable skill documents, `plugins/unifi/skills/unifi-network/SKILL.md` and `plugins/unifi/skills/unifi-protect/SKILL.md`, are classified `upstream-byte-copy` in `plugins/unifi/PROVENANCE.json`, and neither carries a `compatibility` field. Adding one here would break digest equality with its source, which is the byte-copy rule the whole port is built on, and would move the assessed package's tree fingerprint, which retires the ten-client matrix bound to it. The declaration therefore has to be authored upstream and arrive by re-synchronization; it is queued as such. What this repository can enforce meanwhile, and now does, is that a `compatibility` value, once a skill carries one, must equal the catalog floor.
...
A second job pins the catalog's declared floor, `python>=3.12`, and installs `requests`, `urllib3`, and `pytest` on it, then runs the ported plugin tests. This job is where the Fleet Core slice tests, the site-profile tests, the discovery tests, and the parser-to-parser parity comparison run. The interpreter is pinned to the floor rather than to the newest available one on purpose: the contract admits every later interpreter, and the only version this job can usefully prove is the lowest one the contract admits. A floor that is never exercised is not a floor.

The floor itself is one value in one place, `tests/test_python_floor.py`, and that module is the gate. It fails when the continuous-integration pin, a package README or changelog, this plan, the repository README, or the recorded decision states a different floor from the one it holds; when any of those files stops stating the floor at all; and when a portable skill declares a `compatibility` value that disagrees. That is what makes the floor a checked contract rather than four hand-maintained strings that drift apart between reviews.
