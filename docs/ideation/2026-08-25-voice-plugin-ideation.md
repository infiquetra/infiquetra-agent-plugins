...
Exactly one utterance holds the operator's attention at a time. Higher urgency preempts, equal or lower waits, and the routine cases render as a short tone rather than a sentence. The text is authored by the producer, not extracted by the plugin: the agent writes its own one-line spoken closer as part of its reply, and when that line is missing the plugin plays a "finished, nothing to say" tone rather than guessing which part of the prose mattered. Because the non-speech alternative travels with the utterance, the lowest tier works with no provider configured at all.
