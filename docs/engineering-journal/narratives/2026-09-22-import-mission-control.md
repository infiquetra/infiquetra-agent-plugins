...
- After rebasing onto origin/main, UniFi is authored too, so the marker-rule precondition test no longer has a live derived descriptor to pass. The passing case is a synthetic descriptor whose client extension directory is `com.infiquetra.claude`.
