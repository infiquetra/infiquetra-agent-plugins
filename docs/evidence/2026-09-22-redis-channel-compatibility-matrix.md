...
# Ten-client compatibility matrix — portable redis-channel package (0.5.3)

This repository holds the portable source catalog for Infiquetra Agent Skills
and Agent Plugins. `plugins/redis-channel/` is authored and maintained
here. This document records what happened when the shipped package —
48 files, tree `04df2c22240a78f400aa71efea62c93ebe82ac93c9ec79eb45212f9bed3e84b8` — was put in front
...
- **Isolation.** Each client was handed its own fresh copy of the shipped tree, at 48 files, fingerprinted before and after that client ran. Every copy was unchanged afterwards, so no client added a vendor artifact to the package. Nine clients ran against
...
- **Credentials.** The harness removed every variable matching `REDIS_CHANNEL_` from every subprocess environment before each stage. Nine clients ran unauthenticated in their own empty scratch homes. Cursor Agent ran against the operator's real authenticated home by design, because an isolated home strips its authentication and produces a false failure; its authentication state is recorded only as present. No credential was created, changed, or read into this evidence, and no account identity is published here.
...
| OpenCode | The client engaged with the package and could not fully consume it. Executed: placement, discovery, load. Blocked: invocation. Exited non-zero at: discovery, load. At discovery, the client reported the subcommand this stage uses as unknown. At load, the client reported the subcommand this stage uses as unknown. invocation: OpenCode installs skill units rather than the package, so declared entrypoint(s) sitting outside every declared skill unit have no client-resolved path: 'scripts/integ_test.py', 'scripts/mcp_server.py'. |
| Gemini CLI | The client engaged with the package and could not fully consume it. Executed: placement, discovery, load. Blocked: invocation. invocation: Gemini CLI installs skill units rather than the package, so declared entrypoint(s) sitting outside every declared skill unit have no client-resolved path: 'scripts/integ_test.py', 'scripts/mcp_server.py'. |
| Muse | The client engaged with the package and could not fully consume it. Executed: placement, discovery, load. Blocked: invocation. invocation: Muse installs skill units rather than the package, so declared entrypoint(s) sitting outside every declared skill unit have no client-resolved path: 'scripts/integ_test.py', 'scripts/mcp_server.py'. |
...
| Hermes | The client engaged with the package and could not fully consume it. Executed: placement, discovery, load. Blocked: invocation. invocation: Hermes installs skill units rather than the package, so declared entrypoint(s) sitting outside every declared skill unit have no client-resolved path: 'scripts/integ_test.py', 'scripts/mcp_server.py'. |
...
```json
{
  ...
  "package": {
    ...
    "version": "0.5.3",
    "file_count": 48,
    "tree_sha256": "04df2c22240a78f400aa71efea62c93ebe82ac93c9ec79eb45212f9bed3e84b8"
  },
  "method": {
    "stages": [
      "placement",
    ...
    "isolation": "Each client was handed its own fresh copy of the shipped tree, at 48 files, fingerprinted before and after that client ran. Every copy was unchanged afterwards, so no client added a vendor artifact to the package.",
    "credentials": "The harness removed every variable matching `REDIS_CHANNEL_` from every subprocess environment before each stage. Nine clients ran unauthenticated in their own empty scratch homes. Cursor Agent ran against the operator's real authenticated home by design, because an isolated home strips its authentication and produces a false failure; its authentication state is recorded only as present. No credential was created, changed, or read into this evidence, and no account identity is published here.",
    "network": "The invocation stage runs each declared entrypoint's credential-free `--help` action, which parses arguments and exits without a network call. The other three stages run each client's own commands, so a client that reaches its own service does so on its own account; Cursor Agent's three stages are model prompts, which is why it is the one client assessed against the real authenticated home."
  },
  "clients": [
    {
      ...
      "stages": {
        "placement": {
          ...
          "evidence": "Ran 1 command(s); exit status 0. The client's output named the package name, version 0.5.3, 1 of 1 declared skill unit(s)."
        },
        "discovery": {
          "result": "executed",
          "command": "claude --plugin-dir <package> plugin list",
        ...
        "load": {
          "result": "executed",
          "command": "claude --plugin-dir <package> plugin details redis-channel",
          "commands": [
            {
              "command": "claude --plugin-dir <package> plugin details redis-channel",
              "exit_status": 0
        ...
        "invocation": {
          "result": "executed",
          "command": "<python> <package>/scripts/integ_test.py --help",
          "commands": [
            {
              "command": "<python> <package>/scripts/integ_test.py --help",
              "exit_status": 0
            ...
            {
              "command": "<python> <package>/scripts/mcp_server.py --help",
              "exit_status": 0
            }
          ],
    ...
    },
    {
      ...
      "stages": {
        ...
        "invocation": {
          "result": "executed",
          "command": "<python> <client-home>/.qwen/extensions/redis-channel/scripts/integ_test.py --help",
          "commands": [
            {
              "command": "<python> <client-home>/.qwen/extensions/redis-channel/scripts/integ_test.py --help",
              "exit_status": 0
            ...
            {
              "command": "<python> <client-home>/.qwen/extensions/redis-channel/scripts/mcp_server.py --help",
              "exit_status": 0
            }
          ],
          "evidence": "Ran 2 command(s); exit status 0, 0. The client's output named the package name."
    ...
    },
    {
      ...
      "stages": {
        ...
        "load": {
          "result": "executed",
          "command": "grok plugin details redis-channel",
          "commands": [
            {
              "command": "grok plugin details redis-channel",
              "exit_status": 0
        ...
        "invocation": {
          "result": "blocked",
          "command": "<python> <client-home>/.grok/installed-plugins/<plugin-id>/scripts/integ_test.py --help",
    ...
    },
    {
      ...
      "stages": {
        "placement": {
          "result": "executed",
          "command": "cp -R <package>/skills/redis-channel <client-home>/.agents/skills/",
          "commands": [
            {
              "command": "cp -R <package>/skills/redis-channel <client-home>/.agents/skills/",
              "exit_status": 0
        ...
        "invocation": {
          "result": "blocked",
          "reason": "OpenCode installs skill units rather than the package, so declared entrypoint(s) sitting outside every declared skill unit have no client-resolved path: 'scripts/integ_test.py', 'scripts/mcp_server.py'. A stage that half ran did not run, so invocation is blocked in advance."
        }
      },
      "status": "works-through-an-adapter",
      "reason": "The client engaged with the package and could not fully consume it. Executed: placement, discovery, load. Blocked: invocation. Exited non-zero at: discovery, load. At discovery, the client reported the subcommand this stage uses as unknown. At load, the client reported the subcommand this stage uses as unknown. invocation: OpenCode installs skill units rather than the package, so declared entrypoint(s) sitting outside every declared skill unit have no client-resolved path: 'scripts/integ_test.py', 'scripts/mcp_server.py'."
    },
    {
      ...
      "stages": {
        "placement": {
          "result": "executed",
          "command": "gemini skills link <package>/skills/redis-channel",
          "commands": [
            {
              "command": "gemini skills link <package>/skills/redis-channel",
              "exit_status": 0
        ...
        "invocation": {
          "result": "blocked",
          "reason": "Gemini CLI installs skill units rather than the package, so declared entrypoint(s) sitting outside every declared skill unit have no client-resolved path: 'scripts/integ_test.py', 'scripts/mcp_server.py'. A stage that half ran did not run, so invocation is blocked in advance."
        }
      },
      "status": "works-through-an-adapter",
      "reason": "The client engaged with the package and could not fully consume it. Executed: placement, discovery, load. Blocked: invocation. invocation: Gemini CLI installs skill units rather than the package, so declared entrypoint(s) sitting outside every declared skill unit have no client-resolved path: 'scripts/integ_test.py', 'scripts/mcp_server.py'."
    },
    {
      ...
      "stages": {
        "placement": {
          "result": "executed",
          "command": "muse skills install <package>/skills/redis-channel --scope user",
          "commands": [
            {
              "command": "muse skills install <package>/skills/redis-channel --scope user",
              "exit_status": 0
        ...
        "load": {
          "result": "executed",
          "command": "muse skills install <package>/skills/redis-channel --scope user --force --json",
          "commands": [
            {
              "command": "muse skills install <package>/skills/redis-channel --scope user --force --json",
              "exit_status": 0
        ...
        "invocation": {
          "result": "blocked",
          "reason": "Muse installs skill units rather than the package, so declared entrypoint(s) sitting outside every declared skill unit have no client-resolved path: 'scripts/integ_test.py', 'scripts/mcp_server.py'. A stage that half ran did not run, so invocation is blocked in advance."
        }
      },
      "status": "works-through-an-adapter",
      "reason": "The client engaged with the package and could not fully consume it. Executed: placement, discovery, load. Blocked: invocation. invocation: Muse installs skill units rather than the package, so declared entrypoint(s) sitting outside every declared skill unit have no client-resolved path: 'scripts/integ_test.py', 'scripts/mcp_server.py'."
    },
    {
      ...
      "stages": {
        ...
        "load": {
          "result": "executed",
          "command": "agy plugin validate <client-home>/.gemini/config/plugins/redis-channel",
          "commands": [
            {
              "command": "agy plugin validate <client-home>/.gemini/config/plugins/redis-channel",
              "exit_status": 0
        ...
        "invocation": {
          "result": "executed",
          "command": "<python> <client-home>/.gemini/config/plugins/redis-channel/scripts/integ_test.py --help",
          "commands": [
            {
              "command": "<python> <client-home>/.gemini/config/plugins/redis-channel/scripts/integ_test.py --help",
              "exit_status": 0
            ...
            {
              "command": "<python> <client-home>/.gemini/config/plugins/redis-channel/scripts/mcp_server.py --help",
              "exit_status": 0
            }
          ],
          "evidence": "Ran 2 command(s); exit status 0, 0. The client's output named the package name."
    ...
    },
    {
      ...
      "stages": {
        "placement": {
          "result": "executed",
          "command": "cp -R <package>/skills/redis-channel <client-home>/.hermes/skills/",
          "commands": [
            {
              "command": "cp -R <package>/skills/redis-channel <client-home>/.hermes/skills/",
              "exit_status": 0
        ...
        "invocation": {
          "result": "blocked",
          "reason": "Hermes installs skill units rather than the package, so declared entrypoint(s) sitting outside every declared skill unit have no client-resolved path: 'scripts/integ_test.py', 'scripts/mcp_server.py'. A stage that half ran did not run, so invocation is blocked in advance."
        }
      },
      "status": "works-through-an-adapter",
      "reason": "The client engaged with the package and could not fully consume it. Executed: placement, discovery, load. Blocked: invocation. invocation: Hermes installs skill units rather than the package, so declared entrypoint(s) sitting outside every declared skill unit have no client-resolved path: 'scripts/integ_test.py', 'scripts/mcp_server.py'."
