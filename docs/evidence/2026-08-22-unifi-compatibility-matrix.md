...
The machine-readable fingerprint below was rebound on 2026-09-22, when the
Fleet Core `retry_backoff` bundles in this package were regenerated from the
authored 0.32.0 import. The client results in this document were not re-run.
They describe the assessment of 23 August 2026. A later ten-client run has to
replace them before anyone treats them as evidence of the regenerated bytes.
