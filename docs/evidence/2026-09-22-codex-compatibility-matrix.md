...
and Agent Plugins. `plugins/codex/` is authored and maintained
here. This document records what happened when the shipped package —
19 files, tree `fa3da79e9405ecb03f8549c107bf25864ce3c82ce95222615783772ea7c41903` — was put in front
...
- **Isolation.** Each client was handed its own fresh copy of the shipped tree, at 19 files, fingerprinted before and after that client ran. Every copy was unchanged afterwards, so no client added a vendor artifact to the package. Nine clients ran against
...
- **Credentials.** The harness removed every variable matching `OPENAI_`, `CODEX_` from every subprocess environment before each stage. Nine clients ran unauthenticated in their own empty scratch homes. Cursor Agent ran against the operator's real authenticated home by design, because an isolated home strips its authentication and produces a false failure; its authentication state is recorded only as present. No credential was created, changed, or read into this evidence, and no account identity is published here.
...
| OpenCode | The client engaged with the package and could not fully consume it. Executed: placement, discovery, load. Blocked: invocation. Exited non-zero at: discovery, load. At discovery, the client reported the subcommand this stage uses as unknown. At load, the client reported the subcommand this stage uses as unknown. invocation: OpenCode installs skill units rather than the package, so declared entrypoint(s) sitting outside every declared skill unit have no client-resolved path: 'scripts/codex_delegate.py'. |
| Gemini CLI | The client engaged with the package and could not fully consume it. Executed: placement, discovery, load. Blocked: invocation. invocation: Gemini CLI installs skill units rather than the package, so declared entrypoint(s) sitting outside every declared skill unit have no client-resolved path: 'scripts/codex_delegate.py'. |
| Muse | The client engaged with the package and could not fully consume it. Executed: placement, discovery, load. Blocked: invocation. invocation: Muse installs skill units rather than the package, so declared entrypoint(s) sitting outside every declared skill unit have no client-resolved path: 'scripts/codex_delegate.py'. |
...
| Hermes | The client engaged with the package and could not fully consume it. Executed: placement, discovery, load. Blocked: invocation. invocation: Hermes installs skill units rather than the package, so declared entrypoint(s) sitting outside every declared skill unit have no client-resolved path: 'scripts/codex_delegate.py'. |
...
```json
{
  ...
  "package": {
    ...
    "version": "0.1.5",
    "file_count": 19,
    "tree_sha256": "fa3da79e9405ecb03f8549c107bf25864ce3c82ce95222615783772ea7c41903"
  },
  "method": {
    "stages": [
      "placement",
    ...
    "isolation": "Each client was handed its own fresh copy of the shipped tree, at 19 files, fingerprinted before and after that client ran. Every copy was unchanged afterwards, so no client added a vendor artifact to the package.",
    "credentials": "The harness removed every variable matching `OPENAI_`, `CODEX_` from every subprocess environment before each stage. Nine clients ran unauthenticated in their own empty scratch homes. Cursor Agent ran against the operator's real authenticated home by design, because an isolated home strips its authentication and produces a false failure; its authentication state is recorded only as present. No credential was created, changed, or read into this evidence, and no account identity is published here.",
    "network": "The invocation stage runs each declared entrypoint's credential-free `--help` action, which parses arguments and exits without a network call. The other three stages run each client's own commands, so a client that reaches its own service does so on its own account; Cursor Agent's three stages are model prompts, which is why it is the one client assessed against the real authenticated home."
  },
  "clients": [
    {
      ...
      "stages": {
        "placement": {
          ...
          "evidence": "Ran 1 command(s); exit status 0. The client's output named the package name, version 0.1.5, 0 of 1 declared skill unit(s)."
        },
        "discovery": {
          "result": "executed",
          "command": "claude --plugin-dir <package> plugin list",
        ...
        "load": {
          "result": "executed",
          "command": "claude --plugin-dir <package> plugin details codex",
          "commands": [
            {
              "command": "claude --plugin-dir <package> plugin details codex",
              "exit_status": 0
          ...
          "evidence": "Ran 1 command(s); exit status 0. The client's output named the package name, version 0.1.5, 1 of 1 declared skill unit(s)."
        },
        "invocation": {
          "result": "executed",
          "command": "<python> <package>/scripts/codex_delegate.py --help",
          "commands": [
            {
              "command": "<python> <package>/scripts/codex_delegate.py --help",
              "exit_status": 0
    ...
    },
    {
      ...
      "stages": {
        ...
        "invocation": {
          "result": "executed",
          "command": "<python> <client-home>/.qwen/extensions/codex/scripts/codex_delegate.py --help",
          "commands": [
            {
              "command": "<python> <client-home>/.qwen/extensions/codex/scripts/codex_delegate.py --help",
              "exit_status": 0
    ...
    },
    {
      ...
      "stages": {
        ...
        "load": {
          "result": "executed",
          "command": "grok plugin details codex",
          "commands": [
            {
              "command": "grok plugin details codex",
              "exit_status": 0
        ...
        "invocation": {
          "result": "blocked",
          "command": "<python> <client-home>/.grok/installed-plugins/<plugin-id>/scripts/codex_delegate.py --help",
    ...
    },
    {
      ...
      "stages": {
        "placement": {
          "result": "executed",
          "command": "cp -R <package>/skills/codex-delegate <client-home>/.agents/skills/",
          "commands": [
            {
              "command": "cp -R <package>/skills/codex-delegate <client-home>/.agents/skills/",
              "exit_status": 0
        ...
        "invocation": {
          "result": "blocked",
          "reason": "OpenCode installs skill units rather than the package, so declared entrypoint(s) sitting outside every declared skill unit have no client-resolved path: 'scripts/codex_delegate.py'. A stage that half ran did not run, so invocation is blocked in advance."
        }
      },
      "status": "works-through-an-adapter",
      "reason": "The client engaged with the package and could not fully consume it. Executed: placement, discovery, load. Blocked: invocation. Exited non-zero at: discovery, load. At discovery, the client reported the subcommand this stage uses as unknown. At load, the client reported the subcommand this stage uses as unknown. invocation: OpenCode installs skill units rather than the package, so declared entrypoint(s) sitting outside every declared skill unit have no client-resolved path: 'scripts/codex_delegate.py'."
    },
    {
      ...
      "stages": {
        "placement": {
          "result": "executed",
          "command": "gemini skills link <package>/skills/codex-delegate",
          "commands": [
            {
              "command": "gemini skills link <package>/skills/codex-delegate",
              "exit_status": 0
        ...
        "invocation": {
          "result": "blocked",
          "reason": "Gemini CLI installs skill units rather than the package, so declared entrypoint(s) sitting outside every declared skill unit have no client-resolved path: 'scripts/codex_delegate.py'. A stage that half ran did not run, so invocation is blocked in advance."
        }
      },
      "status": "works-through-an-adapter",
      "reason": "The client engaged with the package and could not fully consume it. Executed: placement, discovery, load. Blocked: invocation. invocation: Gemini CLI installs skill units rather than the package, so declared entrypoint(s) sitting outside every declared skill unit have no client-resolved path: 'scripts/codex_delegate.py'."
    },
    {
      ...
      "stages": {
        "placement": {
          "result": "executed",
          "command": "muse skills install <package>/skills/codex-delegate --scope user",
          "commands": [
            {
              "command": "muse skills install <package>/skills/codex-delegate --scope user",
              "exit_status": 0
        ...
        "load": {
          "result": "executed",
          "command": "muse skills install <package>/skills/codex-delegate --scope user --force --json",
          "commands": [
            {
              "command": "muse skills install <package>/skills/codex-delegate --scope user --force --json",
              "exit_status": 0
        ...
        "invocation": {
          "result": "blocked",
          "reason": "Muse installs skill units rather than the package, so declared entrypoint(s) sitting outside every declared skill unit have no client-resolved path: 'scripts/codex_delegate.py'. A stage that half ran did not run, so invocation is blocked in advance."
        }
      },
      "status": "works-through-an-adapter",
      "reason": "The client engaged with the package and could not fully consume it. Executed: placement, discovery, load. Blocked: invocation. invocation: Muse installs skill units rather than the package, so declared entrypoint(s) sitting outside every declared skill unit have no client-resolved path: 'scripts/codex_delegate.py'."
    },
    {
      ...
      "stages": {
        ...
        "load": {
          "result": "executed",
          "command": "agy plugin validate <client-home>/.gemini/config/plugins/codex",
          "commands": [
            {
              "command": "agy plugin validate <client-home>/.gemini/config/plugins/codex",
              "exit_status": 0
        ...
        "invocation": {
          "result": "executed",
          "command": "<python> <client-home>/.gemini/config/plugins/codex/scripts/codex_delegate.py --help",
          "commands": [
            {
              "command": "<python> <client-home>/.gemini/config/plugins/codex/scripts/codex_delegate.py --help",
              "exit_status": 0
    ...
    },
    {
      ...
      "stages": {
        "placement": {
          "result": "executed",
          "command": "cp -R <package>/skills/codex-delegate <client-home>/.hermes/skills/",
          "commands": [
            {
              "command": "cp -R <package>/skills/codex-delegate <client-home>/.hermes/skills/",
              "exit_status": 0
        ...
        "invocation": {
          "result": "blocked",
          "reason": "Hermes installs skill units rather than the package, so declared entrypoint(s) sitting outside every declared skill unit have no client-resolved path: 'scripts/codex_delegate.py'. A stage that half ran did not run, so invocation is blocked in advance."
        }
      },
      "status": "works-through-an-adapter",
      "reason": "The client engaged with the package and could not fully consume it. Executed: placement, discovery, load. Blocked: invocation. invocation: Hermes installs skill units rather than the package, so declared entrypoint(s) sitting outside every declared skill unit have no client-resolved path: 'scripts/codex_delegate.py'."
